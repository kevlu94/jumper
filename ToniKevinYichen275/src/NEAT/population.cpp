...
bool Population::epoch(int generation) {
	...
	*/
	...
	std::cerr<<"Number of Species: "<<num_species<<std::endl;
	std::cerr<<"compat_thresh: "<<compat_threshold<<std::endl;
	...
	std::cerr<<"Generation "<<generation<<": "<<"overall_average = "<<overall_average<<std::endl;
	...
	for(curspecies=sorted_species.begin();curspecies!=sorted_species.end();++curspecies) {
		...
		std::cerr<<"orig fitness of Species"<<(*curspecies)->id<<"(Size "<<(*curspecies)->organisms.size()<<"): "<<(*((*curspecies)->organisms).begin())->orig_fitness<<" last improved "<<((*curspecies)->age-(*curspecies)->age_of_last_improvement)<<std::endl;
	}
	...
	if (((*(((*curspecies)->organisms).begin()))->orig_fitness)>
		highest_fitness) {
			...
			highest_last_changed=0;
			std::cerr<<"NEW POPULATION RECORD FITNESS: "<<highest_fitness<<std::endl;
		}
	else {
		++highest_last_changed;
		std::cerr<<highest_last_changed<<" generations since last population fitness record: "<<highest_fitness<<std::endl;
	}
